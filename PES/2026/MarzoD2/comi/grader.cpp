...
static int JuanReturnArray[QMAX];
...
void Guardar(int place , int bit) {
	if(place < 0 || place >= SERVERSIZE) {
		printf("Respuesta Incorrecta [1]\n");
		exit(1);
	}
	if(ServerSaved[place] == 1) {
		printf("Respuesta Incorrecta [2]\n");
		exit(1);
	}
	if(bit < 0 || bit > 1) {
		printf("Respuesta Incorrecta [3]\n");
		exit(1);
	}
	ServerSaved[place] = 1;
	ServerBit[place] = bit;
...
int Leer(int place) {
	if(place < 0 || place >= SERVERSIZE) {
		printf("Respuesta Incorrecta [5]\n");
		exit(1);
	}
	Asktotal++;
	if(Asktotal > ASKMAX) {
		printf("Respuesta Incorrecta [6]\n");
		exit(1);
	}
...
int main(void) {
	...
	for(i = 0 ; i < Q ; i++) {
		...
		for(j = 0 ; j < k ; j++) {
			...
			scanf("%d" , &city);
			Asktotal = 0;
			int Juanreturn = Juan(city);
			if(Juanreturn < 0 || Juanreturn >= N) {
				printf("Respuesta Incorrecta [4]\n");
				exit(1);
			}
			JuanReturnArray[j] = Juanreturn;
		}
		for(j = 0 ; j < k ; j++) {
			if(j > 0) printf(" ");
			printf("%d" , JuanReturnArray[j]);
		}
		printf("\n");
	}
	return 0;
